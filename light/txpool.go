...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if tx.IsTitanZApplyTransaction() {
		...
		if err := core.ValidateTitanZApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
	...
	if tx.IsTitanXApplyTransaction() {
		copyState := pool.currentState(ctx).Copy()
		if err := core.ValidateTitanXApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
