...
import (
	...
	"github.com/titanchain/titanchain/contracts/trc21issuer"
	"github.com/titanchain/titanchain/contracts/trc21issuer/simulation"
...
func airDropTokenToAccountNoTitan() {
...
func testTransferTRC21TokenWithAccountNoTitan() {
	client, err := ethclient.Dial(simulation.RpcEndpoint)
	if err != nil {
		fmt.Println(err, client)
	}

	// access to address which received token trc20 but dont have titan
...
func main() {
	...
	for i := 0; i < 10000000; i++ {
		airDropTokenToAccountNoTitan()
		fmt.Println("Finish airdrop token to a account")
		testTransferTRC21TokenWithAccountNoTitan()
		fmt.Println("Finish transfer trc21 token with a account no titan")
		testTransferTrc21Fail()
		fmt.Println("Finish testing ! Success transferAmount token trc20 with a account no titan")
	}
