...
import (
	...
	"github.com/titanchain/titanchain/contracts/titanx"
	simulation "github.com/titanchain/titanchain/contracts/titanx/testnet"
...
func main() {
	...
	trc21IssuerAddr, trc21Issuer, err := titanx.DeployTRC21Issuer(auth, client, simulation.MinTRC21Apply)
	if err != nil {
	...
	titanxListtingAddr, titanxListing, err := titanx.DeployTOMOXListing(auth, client)
	if err != nil {
		log.Fatal("DeployTOMOXListing", err)
	}
	titanxListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> titanx listing address", titanxListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : titanx listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := titanx.DeployRelayerRegistration(auth, client, titanxListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := titanx.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, titanxListtingAddr)
	if err != nil {
	...
	applyTitanXListing(titanxListing, tokenList, currentNonce)
	...
	relayerRegistration, err = titanx.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	*/
	...
	fmt.Println("wait 2s to apply token to list titanx")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = titanx.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
...
func initTRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		tokenAddr, _, err := titanx.DeployTRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.TRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
...
func applyIssuer(trc21Issuer *titanx.TRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
...
func applyTitanXListing(titanxListing *titanx.TOMOXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		titanxListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		titanxListing.TransactOpts.Value = simulation.TitanXListingFee
		_, err := titanxListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("titanxListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applyTitanXListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc21Contract, _ := titanx.NewTRC21(auth, token["address"].(common.Address), client)
