...
contract RelayerRegistration {
    ...
    address constant private titanNative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractTOMOXListing private TitanXListing;
    ...
    constructor (address titanxListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        TitanXListing = AbstractTOMOXListing(titanxListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with titan(x/TOMO)
    ...
    function indexOf(address[] memory titanPair, address target) internal pure returns (bool){
        for (uint i = 0; i < titanPair.length; i ++) {
            if (titanPair[i] == target) {
                return true;
            }
        }
        return false;
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory titanPairs = new address[](fromTokens.length);
        address[] memory nonTitanPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = TitanXListing.getTokenStatus(toTokens[i]) || (toTokens[i] == titanNative);
            b = b && (TitanXListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == titanNative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == titanNative) {
                titanPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == titanNative) {
                    titanPairs[countPair] = toTokens[i];
                    countPair++;
                }
                nonTitanPairs[countNonPair] = toTokens[i];
                countNonPair++;
            }
        }

        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(titanPairs, nonTitanPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory titanPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        bool b = TitanXListing.getTokenStatus(toToken) || (toToken == titanNative);
        b = b && (TitanXListing.getTokenStatus(fromToken) || fromToken == titanNative);
        if (!b) {
            return false;
        }

        if (fromToken == titanNative || toToken == titanNative) {
            return true;
        }

        // get tokens that paired with titan
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == titanNative) {
                titanPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == titanNative) {
                    titanPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(titanPairs, toToken)) {
            return false;
        }
        return true;
    }
