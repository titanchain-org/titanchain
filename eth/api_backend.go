...
import (
	...
	stateDatabase "github.com/titanchain/titanchain/core/state"
...
func (b *EthApiBackend) GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, titanxState *tradingstate.TradingStateDB, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error) {
	state.SetBalance(msg.From(), math.MaxBig256)
	vmError := func() error { return nil }

	context := core.NewEVMContext(msg, header, b.eth.BlockChain(), nil)
	return vm.NewEVM(context, state, titanxState, b.eth.chainConfig, vmCfg), vmError, nil
}
...
func (b *EthApiBackend) GetOrderNonce(address common.Hash) (uint64, error) {
	titanxService := b.eth.GetTitanX()
	if titanxService != nil {
		author, err := b.GetEngine().Author(b.CurrentBlock().Header())
		if err != nil {
			return 0, err
		}
		titanxState, err := titanxService.GetTradingState(b.CurrentBlock(), author)
		if err != nil {
			return 0, err
		}
		return titanxState.GetNonce(address), nil
	}
	return 0, errors.New("cannot find titanx service")
}

func (b *EthApiBackend) TitanxService() *titanx.TitanX {
	return b.eth.TitanX
}

func (b *EthApiBackend) LendingService() *titanxlending.Lending {
