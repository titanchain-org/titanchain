...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	*/
	...
	if tx.IsTitanZApplyTransaction() {
		...
		return ValidateTitanZApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}

	// validate balance slot, token decimal for TitanX
	if tx.IsTitanXApplyTransaction() {
		copyState := pool.currentState.Copy()
		return ValidateTitanXApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:]))
	}
	return nil
}
