...
func (pool *LendingPool) validateBalance(cloneStateDb *state.StateDB, cloneLendingStateDb *lendingstate.LendingStateDB, tx *types.LendingTransaction, collateralToken common.Address) error {
	...
	lendingTokenDecimal, err := titanXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.LendingToken())
	if err != nil {
	...
	tradingStateDb, err := titanXServ.GetTradingState(pool.chain.CurrentBlock(), author)
	if err != nil {
	...
	if collateralToken.String() != lendingstate.EmptyAddress {
		collateralTokenDecimal, err = titanXServ.GetTokenDecimal(pool.chain, cloneStateDb, collateralToken)
		if err != nil {
	...
	if lendTokenTOMOPrice == nil || lendTokenTOMOPrice.Sign() == 0 {
		if tx.LendingToken().String() == common.TitanNativeAddress {
			lendTokenTOMOPrice = common.BasePrice
		} else {
			lendTokenTOMOPrice, err = lendingServ.GetMediumTradePriceBeforeEpoch(pool.chain, cloneStateDb, cloneTradingStateDb, tx.LendingToken(), common.HexToAddress(common.TitanNativeAddress))
			if err != nil {
				return err
			}
		}
	}
	isTitanXLendingFork := pool.chain.Config().IsTIPTitanXLending(pool.chain.CurrentHeader().Number)
	if err := lendingstate.VerifyBalance(isTitanXLendingFork,
		cloneStateDb,
		cloneLendingStateDb,
		tx.Type(),
		tx.Side(),
