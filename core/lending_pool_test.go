...
import (
	...
	"github.com/titanchain/titanchain/crypto/sha3"
...
func getLendingNonce(userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "titanx_getLendingOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func TestSendLending(t *testing.T) {
	...
	for true {
		...
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, USDAddress, common.HexToAddress(common.TitanNativeAddress), new(big.Int).Mul(_1E8, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		...
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, BTCAddress, common.HexToAddress(common.TitanNativeAddress), new(big.Int).Mul(_1E18, big.NewInt(1)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		...
		// amount 1000 TOMO
		testSendLending(key, nonce, common.HexToAddress(common.TitanNativeAddress), common.Address{}, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.TitanNativeAddress), BTCAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		...
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.TitanNativeAddress), ETHAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
	}
}
