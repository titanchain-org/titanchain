...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, tradingState *tradingstate.TradingStateDB, cfg vm.Config, balanceFee map[common.Address]*big.Int) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		if tx.IsTitanZApplyTransaction() {
			...
			if err := ValidateTitanZApplyTransaction(p.bc, block.Number(), copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
		...
		if tx.IsTitanXApplyTransaction() {
			copyState := statedb.Copy()
			if err := ValidateTitanXApplyTransaction(p.bc, block.Number(), copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				return nil, nil, 0, err
			}
		}
...
func ApplyTransaction(config *params.ChainConfig, tokensFee map[common.Address]*big.Int, bc *BlockChain, author *common.Address, gp *GasPool, statedb *state.StateDB, titanxState *tradingstate.TradingStateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, uint64, error, bool) {
	...
	if tx.To() != nil && tx.To().String() == common.TradingStateAddr && config.IsTIPTitanX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}
	if tx.To() != nil && tx.To().String() == common.TitanXLendingAddress && config.IsTIPTitanX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}
	if tx.IsTradingTransaction() && config.IsTIPTitanX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}

	if tx.IsLendingFinalizedTradeTransaction() && config.IsTIPTitanX(header.Number) {
		return ApplyEmptyTransaction(config, statedb, header, tx, usedGas)
	}
	...
	vmenv := vm.NewEVM(context, statedb, titanxState, config, cfg)
