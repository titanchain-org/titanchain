...
// blockChain_titanx add order state
type blockChainTitanx interface {
...
type OrderPool struct {
	...
	pendingState      *tradingstate.TitanXManagedState // Pending state tracking virtual nonces
...
func NewOrderPool(chainconfig *params.ChainConfig, chain blockChainTitanx) *OrderPool {
...
func (pool *OrderPool) reset(oldHead, newblock *types.Block) {
	if !pool.chainconfig.IsTIPTitanX(pool.chain.CurrentBlock().Number()) || pool.chain.Config().Posv == nil || pool.chain.CurrentBlock().NumberU64() <= pool.chain.Config().Posv.Epoch {
		return
	}
...
func (pool *OrderPool) State() *tradingstate.TitanXManagedState {
	pool.mu.RLock()
...
func (pool *OrderPool) validateOrder(tx *types.OrderTransaction) error {
	...
	cloneTitanXStateDb := pool.currentOrderState.Copy()
	...
	if !tx.IsCancelledOrder() {
		...
		if orderType == OrderTypeLimit {
			...
			titanXServ := posvEngine.GetTitanXService()
			if titanXServ == nil {
				return fmt.Errorf("titanx not found in order validation")
			}
			baseDecimal, err := titanXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.BaseToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get baseDecimal. err: %v", err)
			}
			quoteDecimal, err := titanXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.QuoteToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get quoteDecimal. err: %v", err)
			}
			if err := tradingstate.VerifyBalance(cloneStateDb, cloneTitanXStateDb, tx, baseDecimal, quoteDecimal); err != nil {
				return err
			}
		}
	...
	if !tx.IsCancelledOrder() {
		...
		originOrder := cloneTitanXStateDb.GetOrder(tradingstate.GetTradingOrderBookHash(tx.BaseToken(), tx.QuoteToken()), common.BigToHash(new(big.Int).SetUint64(tx.OrderID())))
...
func (pool *OrderPool) addTx(tx *types.OrderTransaction, local bool) error {
	if !pool.chainconfig.IsTIPTitanX(pool.chain.CurrentBlock().Number()) {
		return nil
	}
	tx.CacheHash()
