...
import (
	...
	"github.com/titanchain/titanchain/consensus/misc"
	"github.com/titanchain/titanchain/consensus/posv"
	"github.com/titanchain/titanchain/contracts"
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	var titanxState *tradingstate.TradingStateDB
	var lendingState *lendingstate.LendingStateDB
	if self.config.Posv != nil {
		titanX := self.eth.GetTitanX()
		titanxState, err = titanX.GetTradingState(parent, author)
		if err != nil {
			log.Error("Failed to get titanx state ", "number", parent.Number(), "err", err)
			return err
		}
		lending := self.eth.GetTitanXLending()
...
func (self *worker) commitNewWork() {
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.Posv != nil && self.chain.Config().IsTIPTitanX(header.Number) {
			titanX := self.eth.GetTitanX()
			titanXLending := self.eth.GetTitanXLending()
			if titanX != nil && header.Number.Uint64() > self.config.Posv.Epoch {
				if header.Number.Uint64()%self.config.Posv.Epoch == 0 {
					err := titanX.UpdateMediumPriceBeforeEpoch(header.Number.Uint64()/self.config.Posv.Epoch, work.tradingState, work.state)
					if err != nil {
				...
				//https://github.com/titanchain/titanchain-v1/pull/416
				if header.Number.Uint64()%self.config.Posv.Epoch != 0 {
					...
					tradingTxMatches, tradingMatchingResults = titanX.ProcessOrderPending(header, self.coinbase, self.chain, tradingOrderPending, work.state, work.tradingState)
					...
					lendingInput, lendingMatchingResults = titanXLending.ProcessOrderPending(header, self.coinbase, self.chain, lendingOrderPending, work.state, work.lendingState, work.tradingState)
					...
					if header.Number.Uint64()%self.config.Posv.Epoch == common.LiquidateLendingTradeBlock {
						updatedTrades, liquidatedTrades, autoRepayTrades, autoTopUpTrades, autoRecallTrades, err = titanXLending.ProcessLiquidationData(header, self.chain, work.state, work.tradingState, work.lendingState)
						if err != nil {
				...
				if len(tradingTxMatches) > 0 {
					...
					tx := types.NewTransaction(nonce, common.HexToAddress(common.TitanXAddr), big.NewInt(0), txMatchGasLimit, big.NewInt(0), txMatchBytes)
				...
				if len(lendingInput) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					lendingTx := types.NewTransaction(nonce, common.HexToAddress(common.TitanXLendingAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), lendingDataBytes)
				...
				if len(updatedTrades) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					finalizedTx := types.NewTransaction(nonce, common.HexToAddress(common.TitanXLendingFinalizedTradeAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), finalizedTradeData)
		...
		TitanxStateRoot := work.tradingState.IntermediateRoot()
		LendingStateRoot := work.lendingState.IntermediateRoot()
		txData := append(TitanxStateRoot.Bytes(), LendingStateRoot.Bytes()...)
...
func (env *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		// validate minFee slot for TitanZ
		if tx.IsTitanZApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateTitanZApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("TitanZApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
		// validate balance slot, token decimal for TitanX
		if tx.IsTitanXApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateTitanXApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("TitanXApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
