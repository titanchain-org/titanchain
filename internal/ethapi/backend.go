...
type Backend interface {
	...
	TitanxService() *titanx.TitanX
	LendingService() *titanxlending.Lending
	...
	GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, titanxState *tradingstate.TradingStateDB, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error)
	...
	return []rpc.API{
		{
			...
			Namespace: "titanx",
			Version:   "1.0",
			Service:   NewPublicTitanXTransactionPoolAPI(apiBackend, nonceLock),
			Public:    true,
		}, {
			Namespace: "txpool",
			Version:   "1.0",
