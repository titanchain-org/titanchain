...
func loadConfig(file string, cfg *titanConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "titan.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, titanConfig) {
	// Load defaults.
	cfg := titanConfig{
		Eth:         eth.DefaultConfig,
		Shh:         whisper.DefaultConfig,
		TitanX:      titanx.DefaultConfig,
	...
	if ctx.GlobalBool(utils.TitanTestnetFlag.Name) {
		...
		common.TIPTitanXBlock = big.NewInt(0)
		common.TIPTitanXLendingBlock = big.NewInt(0)
		common.TIPTitanXCancellationFeeBlock = big.NewInt(0)
		...
		common.TitanXListingSMC = common.TitanXListingSMCTestNet
	}
	...
	utils.SetTitanXConfig(ctx, &cfg.TitanX, cfg.Node.DataDir)
	return stack, cfg
}
...
func makeFullNode(ctx *cli.Context) (*node.Node, titanConfig) {
	stack, cfg := makeConfigNode(ctx)

	// Register TitanX's OrderBook service if requested.
	// enable in default
	utils.RegisterTitanXService(stack, &cfg.TitanX)
