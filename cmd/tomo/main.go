...
const (
	clientIdentifier = "titan" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the titanchain command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.TitanXEnabledFlag,
		utils.TitanXDataDirFlag,
		utils.TitanXDBEngineFlag,
		utils.TitanXDBConnectionUrlFlag,
		utils.TitanXDBReplicaSetNameFlag,
		utils.TitanXDBNameFlag,
...
func init() {
	// Initialize the CLI app and start titan
	app.Action = titan
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright (c) 2018 Titanchain"
...
// titan is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func titan(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, cfg titanConfig) {
	...
	if _, ok := ethereum.Engine().(*posv.Posv); ok {
		go func() {
			started := false
			slaveMode := ctx.GlobalIsSet(utils.TitanSlaveModeFlag.Name)
